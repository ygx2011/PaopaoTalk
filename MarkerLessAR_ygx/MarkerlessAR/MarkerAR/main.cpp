...
ERROR_CODE process_Image(uchar imgData[],float R[],float T[],bool &isShow)
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////
	{
        ...
        if(istracking == false)
        {
            ...
            orb(im, Mat(), keyPoints_2, descriptors_2);
            ...
            if(keyPoints_2.size()>=10)
            {
                ...
                if(train_p.size()>=4 && train_p.size()>=(int)(0.1*descriptors_1.rows))
                {
                    ...
                    perspectiveTransform( obj_corners, scene_corners, H);
					        ...
					        cv::line( im, scene_corners[0], scene_corners[1], cv::Scalar( 255, 0, 0, 255 ), 2 );
                            cv::line( im, scene_corners[1], scene_corners[2], cv::Scalar( 0, 255, 0, 255 ), 2 );
                            cv::line( im, scene_corners[2], scene_corners[3], cv::Scalar( 0, 0, 255, 255 ), 2 );
                            cv::line( im, scene_corners[3], scene_corners[0], cv::Scalar( 0, 0, 0, 255 ), 2 );
                    
                    estimateTransformToCamera(m_corners_3d, scene_corners, m_camera_matrix, m_dist_coeff, r, t);
                    
                    prev_keyPoints.clear();
                    prev_corners.clear();
                    cvtColor(im, prev_gray, CV_BGRA2GRAY);
                    for(int i=0; i<4; i++)
                    {
                    ...
                    Mat mask = createMask(im.size(), prev_corners);
        ...
        else
        {
            ...
            for(int i=0; i<4; i++)
            {
                if(prev_corners[i].x<=0 || prev_corners[i].x>=im.cols || prev_corners[i].y<=0 || prev_corners[i].y>=im.rows)
                {
                    num++;
                }
            }
            ...
            {
                cvtColor(im, curr_gray, CV_BGRA2GRAY);
                ...
                if(tr_num>=15)
                {
                    ...
                    if(countNonZero(homographyMat) != 0)
                    {
                        ...
                        if(prev_corners.size() >= 4)
                        {
							...
							cv::line( im, prev_corners[0], prev_corners[1], cv::Scalar( 255, 0, 0, 255 ), 2 );
                            cv::line( im, prev_corners[1], prev_corners[2], cv::Scalar( 0, 255, 0, 255 ), 2 );
                            cv::line( im, prev_corners[2], prev_corners[3], cv::Scalar( 0, 0, 255, 255 ), 2 );
                            cv::line( im, prev_corners[3], prev_corners[0], cv::Scalar( 0, 0, 0, 255 ), 2 );
	...
	imshow("MarkerlessAR", im);
	waitKey(5);

	cv::cvtColor(im, im, CV_BGR2RGB);
	memcpy(imgData,im.data,640*480*3);
